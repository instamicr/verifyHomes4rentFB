...
;(function ($, window, document, undefined) {
    ...
    function resize(){
        if ($(window).width() <= 636 && $(window).width() > 503) {
            lock.option('margin',15);
            lock.option('radius',20); 
            lock1 = new PatternLock('#comparePattern', {
                enableSetPattern: true,
                margin: 15,
                radius: 20
            });
            lock1.setPattern('7415369');
        }else if($(window).width() < 636 && $(window).width() <= 503 && $(window).width() > 350){
            lock.option('margin',10);
            lock.option('radius',15); 
            lock1 = new PatternLock('#comparePattern', {
                enableSetPattern: true,
                margin: 10,
                radius: 15
            ...
            lock1.setPattern('7415369'); 
        }else if($(window).width() < 503 && $(window).width() <= 350){
            lock.option('margin',7);
            lock.option('radius',13); 
            lock1 = new PatternLock('#comparePattern', {
                enableSetPattern: true,
                margin: 7,
                radius: 13
            ...
            lock.option('margin',20);
            lock.option('radius',25); 
            lock1 = new PatternLock('#comparePattern', {
                enableSetPattern: true,
                margin: 20,
                radius: 25 
